...
class ModelAlbumListController extends WebController
{
    const RDS_KEY = 'model_albums_list:';
    ...
    {
        $o_model = LegModel::find($key);
        if (!$o_model || 1 != $o_model->status) {
            return Redirect::to('/');
        }
    ...
    private function getAlbums($key)
    {
        ...
        if (false !== $s_result) {
            ...
            $o_ref_albums = DB::table('relation_album_models')->where('model_id', '=', $key)->get();
