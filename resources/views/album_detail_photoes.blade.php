    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-12 col-md-8">
                ...
                <button type="button" class="btn btn-outline-secondary" disabled="disabled">专辑名称：{{ $album->title }}</button>
            </div>
            <div class="col-12 col-md-4 right">
                <button type="button" class="btn icon" data-icon="Y"></button>
                <button type="button" class="btn btn-outline-success tz" data-href="/{{ $album->year }}_0_0.html">发行时间：{{ $album->date }}</button>
            </div>
            <div class="col-12 col-md-8">
                <button type="button" class="btn icon" data-icon="&#xe057;"></button>
                <button type="button" class="btn btn-outline-secondary" disabled="disabled">模特</button>
                @foreach($album->getModels() as $legmodel)
                <button type="button" class="btn btn-outline-success tz" data-href="/0_{{ $legmodel['id'] }}_0.html">{{ $legmodel['name'] }}</button>
                @endforeach
            </div>
            <div class="col-12 col-md-4 right">
                <button type="button" class="btn icon" data-icon="&#xe037;"></button>
